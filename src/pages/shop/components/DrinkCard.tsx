...
const DrinkCard = memo(({
  ...
  return (
    <div className='flex flex-col align-middle w-[200px] gap-2 relative'>
      {item?.image ? (
        <img
          src={item?.image}
          alt=""
      ...
      <div>{item.drinks.map((drink, index) => (
        <div key={index} className='flex justify-between'>
          <div className='truncate'>{drink.name}</div>
          <div>{formatMoney(drink?.price)}</div>
        </div>
      ))}</div>
      <hr />
      {/* Action */}
